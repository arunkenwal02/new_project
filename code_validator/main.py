...
#                     for f in files:
#                         if f.get("filename") == "src/model_v1.py":
#                             ids.append(event["id"])
...
def get_push_history(repo_url: str = Query(...),push_id1: str = Query(...),push_id2: str = Query(...)):    # Parse GitHub repo URL
    repo_info = get_repo_info(repo_url)
    commit_list = get_commit_list(repo_info, push_id1, push_id2)
    print(commit_list)
    diff_text = get_commit_diff()
    clean_code = get_clean_code(diff_text)
    return clean_code

@app.get('/summery')
def summery():
    data = get_commit_diffs()
    smry = create_summary(data)
    return smry
    ...
    if os.path.exists(state_file):
        try:
            with open(state_file, "r", encoding="utf-8") as f:
                state = json.load(f)
                last_push_id = state.get("push_id")
                last_sha = state.get("latest_sha")
        except (json.JSONDecodeError, FileNotFoundError):
            last_push_id, last_sha = None, None

    # Skip if already processed
    # if data[1] == last_push_id and sha == last_sha:
    #     return {"message": f"ℹ️ Test cases for push id {data[1]} are already created."}

    py_content = get_file_content("src/model_v1.py", sha)
    if not py_content:
